...
do
  ...
  reported_issues="$(gh issue -R yottta/ddns list --search "\"${ticket_title}\"" --state "all" --json number)"
...
#  gh issue create --repo yottta/ddns --label "govulncheck" --title "${ticket_title}" --body-file ticket_content
  rm ticket_content
